...
# constants
args = edict(
    RUN = f'linear_{DATASET}_Simsiam+JSD',
    BATCH_SIZE = 128,
    ...
    NUM_WORKERS = 8,
    CLASSES = 1000,
    ACCURACY = 'micro' # macro for Flower, other datasets micro according to BYOL paper
)
#custom Callbacks
...
class TransferLearner(pl.LightningModule):
    ...
    def configure_optimizers(self):
        sgd = torch.optim.Adam(self.fc.parameters(), lr=self.lr)# momentum=0.9, weight_decay=0.)
...
class MyImageNetModule(pl.LightningDataModule):
    def __init__(self, data_dir: str = "/home/data", dataset='CIFAR10', num_workers=8, batch_size=128, image_size=225, classes=None):
        super().__init__()
        self.data_dir = data_dir
        # according to BYOL do not use augmentations. Confirm
        self.train_transform = transforms.Compose([
            transforms.RandomResizedCrop((image_size, image_size)),
            #RandomRe((image_size, image_size))
        ...
        self.dataset_type = datasets.ImageFolder if 'CIFAR' not in dataset else eval(f'datasets.{dataset}')
        self.classes = classes
        self.batch_size = batch_size
    
    def prepare_data(self):
        # download
        if self.dataset_type is not datasets.ImageFolder:
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            if self.dataset_type is not datasets.ImageFolder:
                ...
                self.train_dataset = self.dataset_type(f'{self.data_dir}/{self.dataset}/train', transform=self.train_transform)
                self.val_dataset = self.dataset_type(f'{self.data_dir}/{self.dataset}/val', transform=self.transform)
                if self.dataset == 'Imagenet' and self.classes == 100:
                    with open('imagenet100.txt', 'r') as f:
                        allowed_classes = [line.strip() for line in f]
                    classes = [c for c in self.train_dataset.classes if c in allowed_classes]
                    classes.sort()
                    assert len(classes) == 100
                    class_to_idx = {cls_name: i for i, cls_name in enumerate(classes)}
                    for dataset in [self.train_dataset, self.val_dataset]:
                        samples = dataset.make_dataset(dataset.root, class_to_idx, dataset.extensions)
        ...
        print('Test len: ', len(self.val_dataset))
...
if __name__ == '__main__':
    ...
    model = TransferLearner(
        ...
        use_momentum = not (PATH is not None and 'simsiam' in PATH)
