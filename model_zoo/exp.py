...
class JSD(nn.Module):
    ...
    def __call__(self, logits1, logits2, targets):
...
class Experiment(nn.Module):
    ...
    def forward(
        ...
        #target_pred_one = self.online_predictor(target_proj_one)
        #target_pred_two = self.online_predictor(target_proj_two)
        #D_pred_teacher1 = loss_fn(online_proj_one, target_proj_one)
        #D_pred_teacher2 = loss_fn(online_proj_two, target_proj_two)
        
        D_pred_proj1 = loss_fn(online_pred_one, target_proj_two.detach())
        D_pred_proj2 = loss_fn(online_pred_two, target_proj_one.detach())
        ...
        if target is not None:
            jsd_regularizer = self.jsd(logit1, logit2, target)
       # print(jsd_regularizer)
        #loss_proj = 0.5 * (D_pred_proj1 + D_pred_proj2)
        #loss_teacher = - 0.5 * (D_pred_teacher1 + D_pred_teacher2)
        loss_one = D_pred_proj1 #+  D_pred_teacher1  
        loss_two = D_pred_proj2 #+ D_pred_teacher2 
